	...
	try {
		if (type == 'view') {
			...
			if (approvalstatus == 2 && subsidiary == 1 && countsub != reg){
				form.setScript('customscript_ptc_ue_copy_purchase_contra');				
	...
	if (type == 'edit') {
        ...
        if (subsidiary == 1){
            ...
            for(var i = 1; i <= countsub; i++){		
                ...
                amount = parseFloat(amount) + parseFloat(max_amount);
            }

            record.setFieldValue('custbody_importe_maximo', amount);

            try {
                recordId = nlapiSubmitRecord(record);
            } catch (e) {
                nlapiLogExecution('ERROR', 'Error ', 'NetSuite error: Actualizando importe maximo: ' + e.message);				
            }
        }
    }
}
...
function copy_contra(type){
		...
		try {
			if (type == 'edit')
			{
				...
				if (countsub == reg){
					alert('Los Contratos de Compra ya fueron generados.');
					return true;
				}
				...
				if (countsub > 0){
					...
					var maximumamount = record.getFieldValue('custbody_importe_maximo');
					...
					var seg_autorizante = record.getFieldValue('custbody_seg_autorizante');
				...
				for(var i = 1; i <= countsub; i++){		
					...
					if (nro_contrato_compra == null){
						...
						var location = nlapiLookupField('subsidiary',subsidiary,'custrecord_ubicacion');
						...
						contractNew.setFieldValue('custbody_importe_maximo', max_amount);
						...
						contractNew.setFieldValue('custbody_contrato_auto','T');	
						
						contractNew.setFieldValue('subsidiary', subsidiary);
						contractNew.setFieldValue('department', department);
                        contractNew.setFieldValue('location', location);
						contractNew.setFieldValue('custbody_seg_autorizante', seg_autorizante);
					...
					record.setFieldValue('custbody_importe_maximo', tot_max_amount);
				...
				nlapiSetRedirectURL( 'RECORD', 'purchasecontract', 88173, false );
				alert('Se ha completado la creacion de Contratos de Compras satisfactoriamente.');
